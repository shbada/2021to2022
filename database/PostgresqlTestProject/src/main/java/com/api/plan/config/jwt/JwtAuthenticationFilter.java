...
import com.api.plan.api.dto.MemberLoginReqDto;
...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
        ...
        MemberLoginReqDto memberLoginReqDto = null;

        // 1. username, password 를 받아서
        try {
            memberLoginReqDto = om.readValue(request.getInputStream(), MemberLoginReqDto.class);
        } catch (Exception e) {
            e.printStackTrace();
        }

        System.out.println("JwtAuthenticationFilter : " + memberLoginReqDto);

        // 유저네임패스워드 토큰 생성
        UsernamePasswordAuthenticationToken authenticationToken =
                new UsernamePasswordAuthenticationToken(
                        memberLoginReqDto.getMemberId(),
                        memberLoginReqDto.getPassword()); // 패스워드 관련은 스프링이 해준다.
        ...
        System.out.println("Authentication : " + principalDetailis.getMember().getMemberId());
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
        ...
        String jwtToken = JWT.create()
                ...
                .withClaim("id", principalDetailis.getMember().getIdx())
                .withClaim("username", principalDetailis.getMember().getMemberId())
